...
/**
 * OrcaWaveLogo — Definitive Visual Architecture:
 ...
 *    - Soft organic curved bottom boundary fading into 100% transparency — ZERO box or block appearance.
...
export const OrcaWaveLogo: React.FC<OrcaWaveLogoProps> = ({
  ...
  // LOCKED HOVER TRIGGER — Protected event handlers (DO NOT MODIFY)
  ...
  useEffect(() => {
    ...
    // ── CALM, STABLE BASE OCEAN WATERLINE (CONTINUOUS ACROSS FULL LOGO WIDTH) ──
    ...
    // ── LAYER 1: Continuous Base Ocean Surface & Seawater Body (Real Open Sea, Full Width) ──
    ...
    // ── LAYER 2: Strictly Localized Impact Foam & Surface Disturbance (NO GIANT CONE / TRIANGLE) ──
    ...
    // ── LAYER 4: Articulated Killer Whale with Kinematic Phase-Lag & Active Muscular Arching ──
    ...
    const mainLoop = (now: number) => {
      ...
      try {
        // ── 1. Update Ballistic Particles with Natural Physical Gravity ──
        ...
        // ── 2. Trigger Substantial Directional Particle Bursts ──
        ...
        // ── 3. Render Independent Layers in Physical Order ──
  ...
  return (
    ...
    >
      {/* ── Continuous Traveling Waves (4 Balanced Tiers with Refined Breathing Room) ── */}
      <svg
        viewBox="0 0 36 36"
      ...
      >
        ...
        <g mask={`url(#${maskId})`}>
          {/* Wave Tier 1: Top Crest (#38bdf8 — Sky Cyan) */}
          <path
            d={generateWaveD(7)}
            stroke="#38bdf8"
            strokeWidth="1.8"
          ...
          {/* Wave Tier 2: Middle Swell (#2dd4bf — Emerald Teal) */}
          <path
            d={generateWaveD(15)}
            stroke="#2dd4bf"
            strokeWidth="2.0"
          ...
          {/* Wave Tier 3: Lower Surge (#818cf8 — Indigo Blue) */}
          <path
            d={generateWaveD(23)}
            stroke="#818cf8"
            strokeWidth="1.9"
          ...
          {/* Wave Tier 4: Bottom Marine Tide (#818cf8 — Indigo Blue) */}
          <path
            d={generateWaveD(31)}
            stroke="#818cf8"
            strokeWidth="1.8"
      ...
      {/* ── HTML5 Canvas: Organic Open Ocean, Articulated Killer Whale & Substantial Water Splashes (100% Transparent Surroundings, Hover-Only) ── */}
      <canvas
        ref={canvasRef}
        width={128}
        height={128}
