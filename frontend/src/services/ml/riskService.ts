...
function formatRiskPrediction(
  ...
  return {
    ...
    auditProvenance: result.audit ? {
      pipeline: result.pipeline || 'dual-model-precision-risk',
      qualityScore: result.audit.quality_score,
      activeSources: result.audit.active_sources,
      auditedAt: result.audit.audited_at,
      auditWarnings: result.audit.audit_warnings,
      degraded: result.audit.degraded ?? false,
    } : undefined,
  };
}
